...
int main(int argc, char** argv) {
    ...
    const uint64_t max_cycles = parse_u64_plusarg(argc, argv, "max_cycles", 500000ULL);
    const uint64_t hard_limit_half_cycles = (max_cycles + 1000ULL) * 2ULL;

    Vbranch_tb* top = new Vbranch_tb;
    ...
    if (!Verilated::gotFinish()) {
        std::cerr << "VERILATOR_BRANCH: hard limit reached (max_cycles=" << max_cycles << ")\n";
        delete top;
        return 2;
    }
