...
- Vector LD/ST:
  ...
  - **Current RTL limitation:** vector loads/stores only target local memory (`addr[31]==0`). Global vector memory is not implemented.

- Dot: `VDOT` supports INT and FP. It reduces (sums) per-lane products into lane 0; FP16/FP8 accumulate in FP32 then convert back.
...
- **Note:** this opcode is **not implemented** in current RTL; vector ALU uses opcode `0101111`.
...
- funct7 subops (examples):
  ...
  - 0000010 VMIN/VMAX.
  - 0000100 VDOT (INT/FP; reduce-to-lane0 behavior only).
...
- Dot: VDOT (INT/FP; reduce to lane0).
...
- Vector: VADD.{F32,F16,I32}, VSUB, VMUL, VAND, VOR, VSHL, VSHR, VMIN, VMAX, VRCP/VRSQRT (FP32/FP16/FP8), VDOT (INT/FP; reduce to lane0), VCROSS, VSWIZ/VSHUF (scalar mask selectors), VSEL (mask), VCMP.{eq,lt,gt}->{scalar mask rd}, VUNPACK/VPACK (color expand/pack), VATOM add/min/max/xchg/and/or/xor.
