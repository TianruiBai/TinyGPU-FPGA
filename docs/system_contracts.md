...
- **Shared/Scratch BRAM (tile-local)**: implementation-defined base (recommended `0x0000_8000 .. 0x0000_FFFF` in current RTL)
...
**RTL selection rule (as-built CU):** global vs local is selected by `addr[31]` (1=global, 0=local).
...
- bit4: `MPU_FAULT` (any CU reports access violation)
...
### 9.5 MPU (global config, broadcast to all CUs)

These registers are written by the MCU/host and **broadcast to shadow registers in every CU** over the control-path bus.

| Offset | Name | R/W | Description |
| :--- | :--- | :--- | :--- |
| `0x0208` | `MPU_KREGION_BASE` | R/W | Start address of protected kernel code in VRAM |
| `0x020C` | `MPU_KREGION_LIMIT` | R/W | End address of protected kernel code in VRAM |
| `0x0210` | `MPU_CTRL` | R/W | bit0 `ENABLE`, bit1 `EXEC_VRAM_EN`, bit2 `HALT_ON_FAULT`, bit3 `IRQ_ON_FAULT` |

MPU policy summary (CU-side enforcement):
- **iMPU:** execute allowed in broadcast IMEM, or in VRAM only if `MPU_CTRL.EXEC_VRAM_EN=1`.
- **dMPU:** reads allowed from VRAM/IMEM/CSRs; writes allowed to VRAM except the protected kernel window.
- On violation: block the request and record fault status/address/PC in per-CU CSRs.

### 9.6 Per-CU fault/status window

Per-CU CSRs are exposed as a **banked window** for debug and recovery. The MCU selects the core by offset.

| Offset | Name | R/W | Description |
| :--- | :--- | :--- | :--- |
| `0x0300` | `CU_COUNT` | RO | Number of CUs present |
| `0x0304` | `CU_STRIDE` | RO | Byte stride between per-CU windows (default 0x20) |
| `0x0400 + N*0x20` | `CU_FAULT_STATUS` | RO | bit0 `INSTR_FAULT`, bit1 `LOAD_FAULT`, bit2 `STORE_FAULT` |
| `0x0404 + N*0x20` | `CU_FAULT_ADDR` | RO | Address that caused the fault |
| `0x0408 + N*0x20` | `CU_FAULT_PC` | RO | PC of the faulting instruction |
| `0x040C + N*0x20` | `CU_LAST_ERROR` | RO | Optional implementation-defined error code |
