...
- **Vector Opcodes:**
  ...
  - `0010000`: Transfer (`VMOV`, `VBCAST`) **(planned; not implemented in current RTL)**
...
| `MUL` | 000 | `rd = (signed(rs1) * signed(rs2))[31:0]` (R-type only; funct7=`0000001`) |
...
| `000100` | `VDOT` | Dot Product (INT/FP; see notes) |
...
- **Current RTL:** `VDOT` supports **INT and FP**. It multiplies per-lane values and reduces (sums) into lane 0. Lanes 1..N are zeroed.
- **VDOT.I8 (int):** (funct6=`000100`, funct3=`010`) Treats operands as packed INT8. Performs 4-way dot product per 32-bit chunk. Result accumulated into INT32 destination.
- **VDOT.FP16/FP8:** converts operands to FP32, multiplies and accumulates in FP32, then converts the final sum back to FP16/FP8 in lane 0.
- **VDOT.FP32:** FP32 multiply-accumulate, result in lane 0.
- **Predication:** masked-off lanes contribute 0 to the reduction.
...
- `VBCAST.s`, `VINS.s`, `VEXTR.s` are documented here for ISA planning, but there is **no opcode** in the current RTL decode for vector transfer.
...
**Current RTL limitation:** vector loads/stores are supported only to **local memory space** (`addr[31]==0`). Global (external) memory vector accesses are not implemented; use scalar loads/stores for global space.
